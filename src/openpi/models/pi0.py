...
class Pi0(_model.BaseModel):
    ...
    def compute_FM_loss(
        ...
        *,
        ...
        """Flow matching loss for action prediction (Stage 2)."""
        preprocess_rng, noise_rng, time_rng = jax.random.split(rng, 3)
        obs = _model.preprocess_observation(preprocess_rng, obs, train=train)
    ...
    def compute_CE_loss(
        self, rng: at.KeyArrayLike, obs: _model.Observation, train: bool = False
    ) -> at.Float[at.Array, "*b"]:
        """CE loss for subtask generation (Stage 1)."""
        preprocess_rng, _ = jax.random.split(rng)
        obs = _model.preprocess_observation(preprocess_rng, obs, train=train)
        prefix_tokens, prefix_mask, prefix_ar_mask = self.embed_high_level_prefix(obs)
        prefix_attn_mask = make_attn_mask(prefix_mask, prefix_ar_mask)

        targets = jax.nn.one_hot(
            obs.tokenized_prompt[:, 1:],
        ...
        loss_mask = obs.token_loss_mask[:, 1:]
    ...
    def compute_loss(
        ...
        *,
        ...
        obs_stage2: _model.Observation | None = None,
    ) -> at.Float[at.Array, "*b ah"] | dict[str, at.Float[at.Array, "*b"]]:

        if self.training_mode == "both":
            ce_loss = self.compute_CE_loss(rng, obs_stage1, train=train)
            fm_loss = self.compute_FM_loss(rng, obs_stage2, actions, train=train)
            loss = self.ce_loss_weight * ce_loss + jnp.mean(fm_loss)
            return {"loss": loss, "ce_loss": ce_loss, "fm_loss": jnp.mean(fm_loss)}
        elif self.training_mode == "high-level":
            ce_loss = self.compute_CE_loss(rng, obs_stage1, train=train)
            return {"loss": ce_loss, "ce_loss": ce_loss, "fm_loss": jnp.zeros(())}
        elif self.training_mode == "low-level":
            fm_loss = self.compute_FM_loss(rng, obs_stage2, actions, train=train)
            return {"loss": fm_loss, "ce_loss": jnp.zeros(()), "fm_loss": fm_loss}
        else:
